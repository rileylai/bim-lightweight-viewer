...
本專案是和碩聯合科技 AI / Digital Twin 應用工程師實作題：「AI Native 開發程式題目 — BIM 輕量檢視器」。

目標是在有限時間內建立一個純瀏覽器端運作的 BIM 3D scene editor MVP，而不是完整 BIM 規格平台。核心 demo 能力包含 IFC 載入、3D 幾何顯示、物件選取與高亮、TransformControls 移動 / 旋轉 / 縮放、project JSON 儲存與還原，以及後續 GLB / GLTF 匯入與保存。
